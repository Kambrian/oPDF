...
def create31fig(sharex=True, sharey=False, figsize=(8, 8)):
    ...
    plt.setp([a.yaxis for a in ax[:-1]], major_locator=MaxNLocator(
        nbins=nbins, prune='lower', symmetric=True))
    return ax
...
def lnADPDF(lnAD):
    ...
    p = gpar[0][0]*norm.pdf(lnAD, gpar[0][1], gpar[0][2])+gpar[
                            1][0]*norm.pdf(lnAD, gpar[1][1], gpar[1][2])
    return p
...
def ADSurvFunc(AD):
    ...
    gpar = [[0.569,   -0.570,    0.511], [0.431,    0.227,    0.569]] #w, mu, sigma, bi-normal fit
    lnAD = np.log(AD)
    p = gpar[0][0]*norm.sf(lnAD, gpar[0][1], gpar[0][2])+gpar[
                           1][0]*norm.sf(lnAD, gpar[1][1], gpar[1][2])
    return p
...
def get_extent(X, Y):
    ...
    if X.squeeze().ndim == 2:
        ...
        dx = X[1] -X[0]
        dy = Y[1] -Y[0]
    extent = (X.ravel().min()-dx/2, X.ravel().max()+ \
              dx/2, Y.ravel().min()-dy/2, Y.ravel().max()+dy/2)
    return extent
...
def plot_cov_ellipse(cov, pos, nstd=1, fill=False, ax=None, **kwargs):
    ...
    ellip = Ellipse(xy=pos, width=width,
                    height=height, angle=theta, fill=fill, **kwargs)
...
def skeleton(x, y, nbin=10,alpha=0.683,weights=None):
    ...
    xm = np.empty(nbin)
    # ym=xm[:]     #this is wrong! even though id(ym)!=id(xm), and
    # id(ym[0])!=id(xm[0])
    ym = np.empty_like(xm)
