...
nbin=5 #E and L bin, or nbin**2 for single proxies
nx=20 #scan grid
...
if proxy=='':
  ...
  nbinE=nbin**2
  nbinL=1  
  y=[Sample.jointE_like([m,c], nbinE=nbinE, estimator=estimator) for m in x for c in x]  
elif proxy=='L':
  nbinE=1
  nbinL=nbin**2  
  y=[Sample.jointLE_like([m,c], nbinL=nbinL, nbinE=nbinE, estimator=estimator) for m in x for c in x]  
...
def plot_scan(infile):
  ...
  f.close()
  plt.figure()
  extent=list(mm.ravel()[[0,-1]])+list(cc.ravel()[[0,-1]])
  plt.contour(mm,cc,y, levels=[1], colors='k')
  plt.contour(mm,cc,yE, levels=[1], colors='r')
  plt.contour(mm,cc,yL, levels=[1], colors='g')
  plt.contour(mm,cc,yLE, levels=[1], colors='b')
  plt.legend((name, name+'|E', name+'|L', name+'|LE'))
  plt.savefig(outdir+name+'_countour.eps')