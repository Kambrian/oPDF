...
estimator=4
halo=sys.argv[1] #'A2'
cleansample=int(sys.argv[2]) #0,1,2
usetemplate=int(sys.argv[3]) #1
useweight=int(sys.argv[4])#1
scancontour=int(sys.argv[5]) #1
proxycut=None
if len(sys.argv)>6:
  proxycut=sys.argv[6] #r,L
  percentcut=float(sys.argv[7]) #0.8, 0.6...
...
outdir=lib.rootdir+'/plots/paper/extra/Star/Weight%d/'%useweight
if cleansample==0:
  outdir+='Full/'
elif cleansample==1:
  outdir+='Clean/'
...
FullSample=Tracer(halo,DynRMIN=rmin,DynRMAX=rmax,DynDataFile=halo+'star.hdf5')
